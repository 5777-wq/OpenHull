...
```bash
UV_DEFAULT_INDEX=https://pypi.org/simple   uv tool install "git+https://github.com/5777-wq/OpenHull@v1.0.4"
# 没有 uv 时：
pip install "git+https://github.com/5777-wq/OpenHull@v1.0.4"
```
...
**锁定版本安装**（`@v1.0.4`）：可复现、可审计；需要跟踪最新修复时
换成 `@main` 或具体 commit。安装后验证：

```bash
openhull --version        # 应输出 openhull 1.0.4
```
...
- "这个数靠谱吗？" → 指路验证记录：公开基准船（JBC / DTMB 1712 /
  NMRI MP687）+ 书内算例 + 全量测试（当前 381 项；未装 seakeeping
